...
def CalculateDownPressure(nowData:CalculationDataStruct.StructBaseClass, StartDayCount, ToDayCount, handler:CalculationDataHandle.BaseClass):
    ...
    #    该区间内收盘最低的一天即为"低点"。
    # ------------------------------------------------------------------ #
    low_points = []  # 按时间升序记录每次回调的低点价格

    n = len(dataList_asc)
    i = 2  # 至少需要 i-1（昨天）判断连续两天收跌
    ...
    while i < n:
        ...
        look_back         = min(5, i)
        recent_high_close = max(d.close for d in dataList_asc[i - look_back: i])
        cum_drop          = (recent_high_close - cur.close) / recent_high_close * 100
        cond_c            = cum_drop > 3.0
        ...
        # ---- 向后找回调区间最低收盘价 ----
        low_price_in_pullback = cur.close
        rebound_idx           = None
        ...
        while j < n:
            ...
            if day_j.trade_state != 1:
                ...
                continue
            # 更新区间最低收盘
            if day_j.close < low_price_in_pullback:
                low_price_in_pullback = day_j.close
            # 收盘重新站上十日均价，回调结束
            ma10_j = day_j.avg_10
    ...
    # ------------------------------------------------------------------ #
    # 4. 剔除异常低点
    #    低点当日振幅 > 20日平均振幅 × 3 的低点视为异常（如闪崩）予以剔除
    # ------------------------------------------------------------------ #
    filtered_low_points = []
    for lp in low_points:
        # 在升序序列中找收盘价与低点最接近的交易日
        match_day = min(
            (d for d in dataList_asc if d.trade_state == 1),
            key=lambda d: abs(d.close - lp),
            default=None
        )
    ...
    recent_5 = low_points[-5:] if len(low_points) >= 5 else low_points[:]
    ...
    if cluster_price is not None:
        ...
        # 计算相邻低点间涨跌幅，限制 ±5% 防止主升浪干扰
        pct_changes = []
        for k in range(1, len(low_points)):
            prev_lp = low_points[k - 1]
            cur_lp  = low_points[k]
            if prev_lp == 0:
                continue
            pct = (cur_lp - prev_lp) / prev_lp * 100
            pct = max(-5.0, min(5.0, pct))  # 截断到 ±5%
            pct_changes.append(pct)

        avg_pct_change = sum(pct_changes) / len(pct_changes) if pct_changes else 0.0

        # 从最后一个低点按均值涨跌幅外推下一支撑位
        last_low      = low_points[-1]
        support_price = last_low * (1 + avg_pct_change / 100)
        support_type  = (
            f"趋势推算支撑（共{len(low_points)}个低点，"
            f"平均低点涨跌幅{avg_pct_change:.2f}%）"
        )
