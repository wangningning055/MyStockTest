...
def CalculateDownPressurePointList(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass"):
    ...
    #低点数量要大于四个， 低点小于四个直接返回无效值
    if not low_points or len(low_points) < 4:
        return None, None
...
def CalculateDownPressurePointUpRatio(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass"):
    lowPoints, backLow = CalculateDownPressurePointList(nowData, StartDayCount, ToDayCount, handler)
    if lowPoints is not None and backLow is not None:
        return 1
    return 0

    #lower_tend_two_ratio : float        #最近两个低点的涨跌幅
def CalculateDownPressurePointUpRatio_two_ratio(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass"):
    lowPoints, backLow = CalculateDownPressurePointList(nowData, StartDayCount, ToDayCount, handler)
...
def CalculateDownPressurePointUpRatio_tend_days(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass"):
    ...
    if lowPoints is not None and backLow is not None and len(lowPoints) > 2 and len(backLow) >= 1:
        ...
        avg_fix_add = 0
        for single in nowData.dataList_240:
            if single.trade_date == near.trade_date:
                break
            avg_fix_add += 1
        return avg_fix_add
    return -999
...
def CalculateDownPressurePointUpRatio_now_ratio(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass"):
    lowPoints, backLow = CalculateDownPressurePointList(nowData, StartDayCount, ToDayCount, handler)
    if lowPoints is not None and backLow is not None and len(lowPoints) > 2 and len(backLow) >= 1:
        now = nowData.low
        near = lowPoints[0].low
        return ((now - near) / near) * 100
    return -999

    #lower_tend_rebound_now_ratio : float        #最近的反弹点到当前天的涨跌幅
def CalculateDownPressurePointUpRatio_rebound_now_ratio(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass"):
    lowPoints, backLow = CalculateDownPressurePointList(nowData, StartDayCount, ToDayCount, handler)
    if lowPoints is not None and backLow is not None and len(lowPoints) > 2 and len(backLow) >= 1:
        now = nowData.low
        near = backLow[0].low
...
def CalculateDownPressurePointUpRatio_rebound_ratio(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass"):
    lowPoints, backLow = CalculateDownPressurePointList(nowData, StartDayCount, ToDayCount, handler)
    if lowPoints is not None and backLow is not None and len(lowPoints) > 2 and len(backLow) >= 1:
        near = lowPoints[0].low
        last = backLow[0].low
...
def CalculateDownPressurePointUpRatio_rank(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass"):
    lowPoints, backLow = CalculateDownPressurePointList(nowData, StartDayCount, ToDayCount, handler)
    if lowPoints is not None and backLow is not None and len(lowPoints) > 2 and len(backLow) >= 1:
        rebound = backLow[0]
        count = 0
        for low in lowPoints:
            count = count + 1
            if low.trade_date == rebound.trade_date:
                break
        return count
    return -999
...
def CalculateIsCube(nowData:"CalculationDataStruct.StructBaseClass", StartDayCount, ToDayCount, handler:"CalculationDataHandle.BaseClass", amplitude):
    ...
    maxEdgePrice_1 = edge_avg + edge_avg * amplitude
    minEdgePrice_1 = edge_avg - edge_avg * amplitude
    ...
    maxCubePrice_1 = cube_avg + cube_avg * (amplitude)
    minCubePrice_1 = cube_avg - cube_avg * (amplitude)

    maxCubePrice_1_5 = cube_avg + cube_avg * (amplitude * 1.2)
    minCubePrice_1_5 = cube_avg - cube_avg * (amplitude * 1.2)

    maxCubePrice_2 = cube_avg + cube_avg * (amplitude * 1.5)
    minCubePrice_2 = cube_avg - cube_avg * (amplitude * 1.5)
    
    #不能存在任何一个点超过 平均 + 1.5倍的amplitude上下
    index = 0
    for single in nowData.dataList_240:
        if index >= StartDayCount and index <= ToDayCount:
    ...
    #不超过百分之二十的点在 平均 + 1.2倍的amplitude外
    outCount = 0
    targetOutCount = (ToDayCount - StartDayCount) * 0.2
    index = 0
    for single in nowData.dataList_240:
        if index >= StartDayCount and index <= ToDayCount:
            if single.close > maxCubePrice_1_5 or single.close < minCubePrice_1_5:
                outCount += 1
                if outCount >= targetOutCount:
                    return 0
        index += 1
    ...
    if outCount > (ToDayCount - StartDayCount) * 0.3:
        return 0
    #print(f"名字是：{nowData.componyInfo.Name}， 目标幅度：{amplitude}， 开始日期：{newestDay.trade_date}， 结束日期：{oldestDay.trade_date}， 开始结束价格中间价格：{edge_avg}， 箱体中间价格：{cube_avg}， 单倍箱体上沿：{maxCubePrice_1}， 单倍箱体下沿：{minCubePrice_1}")
