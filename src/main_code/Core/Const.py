...
def GetStopRatio(stockCode):
    ...
    #这个日期区间是小幅震荡上涨的
    #计算
    #跌破十日均价认为是开始回调，由这天开始往前遍历，直到跌到低于五日均价的某个幅度（比值为0.98（跌五日均线2%）），认为是低点，开始往前寻找放大量上涨的前一天，那天就是股价波动低点
    #如果股价慢慢回到了五日均线上且没找到放大量上涨的那天，那就开始重新寻找回调点
    #把得到的区间的股价波动低点计算平均涨幅，最近的一次波动低点乘平均涨幅，就是这支股票在最新时间的支撑位
    ...
    #这个日期区间是小幅震荡上涨的
    #计算
    #涨超十日均价认为是开始上涨，由这天开始往前遍历，直到涨到高于十日均价的某个幅度（比值为1.05（超十日均线5%）），认为是高点，开始往前寻找放大量下跌的前一天，那天就是股价波动高点
    #把得到的区间的股价波动高点计算平均涨幅，最近的一次波动高点乘平均涨幅，就是这支股票在最新时间的上压力位
