...
class StructBaseClass :
    ...
    def Init(self, handler: "CalculationDataHandle.BaseClass", stockCode, date, dbData):
        ...
        self.is_one_ban = 1 if (self.open == self.close == self.high == self.low) else 0   #是否一字板

        self.is_short_entity = 1 if abs((self.open - self.close) / self.close) < 0.02  else 0   #是否短实体

        entity_up = 0
        entity_down = 0
        if self.change_Ratio >= 0:
            entity_up = self.close
            entity_down = self.open
        else:
            entity_up = self.open
...
class StructBaseWindowClass :
    ...
    down_pressure_pointUp_upTend : float        #反弹点后的低点没有跌破反弹点前的高点
    ...
    cube_3 : int  # 幅度为3%的箱体
    cube_5 : int  # 幅度为5%的箱体
    cube_10 : int  # 幅度为10%的箱体
    cube_15 : int  # 幅度为15%的箱体
    cube_20 : int  # 幅度为20%的箱体
    cube_30 : int  # 幅度为30%的箱体
    cube_50 : int  # 幅度为50%的箱体
